...
有待发结果时每 5 秒进行轻量状态检查，最长 10 分钟，不调用模型；超时退出，下一次 Hook 或 `retry` 继续。没有待办时不轮询。消息包含渲染参数和完整必要上下文，要求主会话只调用一次 `render_monitor_feedback`；工具不可用时只说明无法展示。用户主动确认后的回传不经过自动建议的空闲闸门。
